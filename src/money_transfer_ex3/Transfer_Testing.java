...
class Transfer {
	...
	public void make_transfer(int s, String threadname) throws InterruptedException {
		...
		if (source_account.hashCode() > dest_account.hashCode()) {
			synchronized (source_account) {
				...
				synchronized (dest_account) {
					...
					System.out.println(threadname + " is done. The source account is " + source_account[0]
							+ " and dest account is " + dest_account[0]);
				}
			}
		} else {
			synchronized (dest_account) {
	...
	public String print() {
		return ("source account  " + source_account[0] + "dest account " + dest_account[0]);
...
public class Transfer_Testing {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		System.out.println(
				"The first accound is " + first_account[0] + ", and the second account is " + second_account[0]);
